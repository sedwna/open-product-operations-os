...
<body>
    ...
    <section class="workspace">
      ...
      <form id="wizard">
        ...
        <section class="page" data-page="3">
          ...
          <label class="switch-row"><span><strong>پنل قابل‌نوشتن محلی</strong><small>ثبت ایده و تصمیم فقط در نشست محلی مجاز باشد.</small></span><input class="switch" type="checkbox" name="writableDashboard" checked></label>
        ...
        <section class="page" data-page="4">
          ...
          <div class="notice">هیچ رمز، کلید دسترسی، دادهٔ مشتری یا نشانی خصوصی در پاسخ‌ها وارد نکنید. اتصال ایجنت توسعه بعداً و با مجوز مستقل فعال می‌شود.</div>
        </section>
  ...
  <script nonce="${escapeHtml(nonce)}">
    ...
    $$('input[name=appMode]').forEach(el=>el.onchange=()=>{const mode=$('input[name=appMode]:checked').value;$('#existing-path-field').hidden=mode!=='existing';$('#existingApplicationPath').required=mode==='existing';$('#applicationFolder').disabled=mode!=='create'});
    ...
    function formData(){const f=new FormData($('#wizard'));return{workspaceParent:f.get('workspaceParent'),operationsFolder:f.get('operationsFolder'),applicationFolder:f.get('applicationFolder'),appMode:f.get('appMode'),existingApplicationPath:f.get('existingApplicationPath'),productName:f.get('productName'),vision:f.get('vision'),targetUsers:String(f.get('targetUsers')||'').split(',').map(x=>x.trim()).filter(Boolean),environments:String(f.get('environments')||'').split(',').map(x=>x.trim()).filter(Boolean),humanAuthorityActorId:f.get('humanAuthorityActorId'),ideaEnabled:$('#ideaEnabled').checked,ideaTitle:f.get('ideaTitle'),ideaDescription:f.get('ideaDescription'),ideaSource:f.get('ideaSource'),ideaPriority:f.get('ideaPriority'),installDependencies:f.has('installDependencies'),initializeGit:f.has('initializeGit'),createInitialCommit:f.has('createInitialCommit'),writableDashboard:f.has('writableDashboard'),gitName:f.get('gitName'),gitEmail:f.get('gitEmail')}}
    function review(){const f=formData();const app=f.appMode==='create'?f.applicationFolder:(f.appMode==='existing'?f.existingApplicationPath:'فعلاً ساخته نمی‌شود');$('#review').innerHTML=[['نام محصول',f.productName],['پوشهٔ عملیات',f.operationsFolder],['کد محصول',app],['کاربران هدف',f.targetUsers.join('، ')],['محیط‌ها',f.environments.join('، ')],['ایدهٔ نخست',f.ideaEnabled?f.ideaTitle:'بعداً ثبت می‌شود']].map(([a,b])=>'<article><small>'+escape(a)+'</small><strong>'+escape(b||'—')+'</strong></article>').join('')}
    function escape(v){const d=document.createElement('div');d.textContent=String(v);return d.innerHTML}
    async function start(){const token=$('meta[name=product-ops-csrf]').content;$('#wizard').hidden=true;$('.topline').hidden=true;$('#running').hidden=false;try{const res=await fetch('/api/apply',{method:'POST',headers:{'content-type':'application/json','x-product-ops-csrf':token},body:JSON.stringify(formData())});const data=await res.json();if(!res.ok)throw new Error(data.error||'شروع راه‌اندازی ناموفق بود.');poll(token)}catch(error){fail(error.message)}}
    async function poll(token){try{const res=await fetch('/api/status',{headers:{'x-product-ops-csrf':token}});const data=await res.json();renderStatus(data);if(data.status==='completed'){dashboardUrl=data.dashboardUrl;$('#running').hidden=true;$('#success').classList.add('show');$('#success-copy').textContent='پوشهٔ عملیات در '+data.result.operationsPath+' ساخته شد. تا چند لحظهٔ دیگر وارد پنل می‌شوید.';setTimeout(()=>location.href=dashboardUrl,1800);return}if(data.status==='failed'){fail(data.error);return}setTimeout(()=>poll(token),700)}catch(error){fail(error.message)}}
    ...
    function fail(message){const box=$('#error-box');box.textContent=message||'راه‌اندازی ناموفق بود.';box.classList.add('show')}
