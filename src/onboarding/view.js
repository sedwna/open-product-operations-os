...
<body>
  ...
  <script nonce="${escapeHtml(nonce)}">
    ...
    async function poll(token){try{const res=await fetch('/api/status',{headers:{'x-product-ops-csrf':token}});const data=await res.json();renderStatus(data);if(data.status==='completed'){dashboardUrl=data.dashboardUrl;$('#running').hidden=true;$('#success').classList.add('show');const developmentReady=data.result.development&&data.result.development.status==='ready';$('#success-copy').textContent='پوشهٔ عملیات در '+data.result.operationsPath+' ساخته شد. '+(developmentReady?'سامانهٔ توسعه نیز آماده و معتبر است. ':'')+'تا چند لحظهٔ دیگر وارد پنل می‌شوید.';setTimeout(()=>location.href=dashboardUrl,1800);return}if(data.status==='failed'){fail(data.error);return}setTimeout(()=>poll(token),700)}catch(error){fail(error.message)}}
