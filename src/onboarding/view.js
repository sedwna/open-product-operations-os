...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;min-height:100vh;background:radial-gradient(circle at 12% 8%,rgba(103,185,154,.18),transparent 26%),radial-gradient(circle at 88% 92%,rgba(242,107,79,.16),transparent 30%),var(--paper);font-family:Tahoma,"Segoe UI",sans-serif;color:var(--ink)}button,input,textarea,select{font:inherit}button{cursor:pointer}
    ...
    .terminal{direction:ltr;text-align:left;background:#132338;color:#dce7ef;border-radius:16px;padding:16px;height:410px;overflow:auto;font:12px/1.8 Consolas,monospace;white-space:pre-wrap}.success{display:none;text-align:center;padding:30px 10px}.success.show{display:block}.success-icon{width:86px;height:86px;border-radius:28px;background:var(--mint);color:white;display:grid;place-items:center;font-size:44px;margin:0 auto 22px;transform:rotate(-5deg)}.success h2{font-size:30px}.success p{color:var(--muted);line-height:2}.error-box{display:none;background:#fff0ed;border:1px solid #f4b4a7;color:#9c3826;border-radius:14px;padding:14px;margin-top:16px;line-height:1.8}.error-box.show{display:block}
...
<body>
    ...
    <section class="workspace">
      ...
      <form id="wizard">
        ...
        <section class="page" data-page="3">
          ...
          <label class="switch-row"><span><strong>ساخت خودکار با کدکس</strong><small>پس از تأیید آمادگی، نقش‌های مهندسی به کدکس متصل و فعال می‌شوند.</small></span><input class="switch" id="automationCodex" type="checkbox" name="automationCodex" checked></label>
          ...
          <label class="switch-row" data-codex-option><span><strong>نصب ابزار خط فرمان در صورت نیاز</strong><small>فقط بستهٔ رسمی کدکس با مدیر بستهٔ نود نصب می‌شود.</small></span><input class="switch" type="checkbox" name="installCodexCli" checked></label>
          <label class="switch-row" data-codex-option><span><strong>تکمیل ورود در صورت نیاز</strong><small>مرورگر باز می‌شود؛ رمز و نشست ورود هرگز داخل پروژه نوشته نمی‌شود.</small></span><input class="switch" type="checkbox" name="authenticateCodex" checked></label>
          ...
          <label class="switch-row"><span><strong>پنل قابل‌نوشتن محلی</strong><small>در صورت فعال‌سازی، ثبت ایده و تصمیم فقط در همین نشست محلی مجاز می‌شود.</small></span><input class="switch" type="checkbox" name="writableDashboard"></label>
        ...
        <section class="page" data-page="4">
          ...
          <div class="notice">هیچ رمز یا کلید دسترسی در پاسخ‌ها وارد نکنید. اگر ساخت خودکار روشن باشد، ورود کدکس بیرون از پروژه نگه داشته می‌شود و عملیات تولید، انتشار و تغییر مخرب همچنان پشت دروازهٔ انسانی می‌ماند.</div>
        </section>
  ...
  <script nonce="${escapeHtml(nonce)}">
    ...
    function renderCodexReadiness(){const c=PREFLIGHT.codex||{};$('#codex-state').textContent=c.status==='ready'?'آمادهٔ اتصال':c.status==='login-required'?'نیازمند ورود':c.status==='not-executable'?'نصب‌شده اما غیرقابل‌اجرا':'نیازمند نصب';$('#codex-state').classList.toggle('waiting',c.status!=='ready');$('#codex-copy').textContent=(c.message||'وضعیت کدکس هنگام اجرا بررسی می‌شود.')+(c.version?' نسخه: '+c.version:'')}
    ...
    function formData(){const f=new FormData($('#wizard'));const appMode=f.get('appMode');return{workspaceParent:f.get('workspaceParent'),operationsFolder:f.get('operationsFolder'),applicationFolder:f.get('applicationFolder'),appMode,existingApplicationPath:f.get('existingApplicationPath'),productName:f.get('productName'),vision:f.get('vision'),targetUsers:String(f.get('targetUsers')||'').split(',').map(x=>x.trim()).filter(Boolean),environments:String(f.get('environments')||'').split(',').map(x=>x.trim()).filter(Boolean),humanAuthorityActorId:f.get('humanAuthorityActorId'),ideaEnabled:$('#ideaEnabled').checked,ideaTitle:f.get('ideaTitle'),ideaDescription:f.get('ideaDescription'),ideaSource:f.get('ideaSource'),ideaPriority:f.get('ideaPriority'),installDependencies:f.has('installDependencies'),initializeGit:f.has('initializeGit'),createInitialCommit:f.has('createInitialCommit'),initializeDevelopmentOs:appMode==='create'||f.has('initializeDevelopmentOs'),automationMode:f.has('automationCodex')?'codex':'manual',installCodexCli:f.has('installCodexCli'),authenticateCodex:f.has('authenticateCodex'),writableDashboard:f.has('writableDashboard'),gitName:f.get('gitName'),gitEmail:f.get('gitEmail')}}
    function review(){const f=formData();const app=f.appMode==='create'?f.applicationFolder:(f.appMode==='existing'?f.existingApplicationPath:'فعلاً ساخته نمی‌شود');$('#review').innerHTML=[['نام محصول',f.productName],['پوشهٔ عملیات',f.operationsFolder],['کد محصول',app],['سامانهٔ توسعه',f.initializeDevelopmentOs?'ساخته و اعتبارسنجی می‌شود':'فعلاً راه‌اندازی نمی‌شود'],['حالت اجرا',f.automationMode==='codex'?'ساخت خودکار با کدکس':'اجرای دستی و کنترل‌شده'],['دسترسی پنل',f.writableDashboard?'قابل‌نوشتن در نشست محلی':'فقط خواندنی'],['کاربران هدف',f.targetUsers.join('، ')],['محیط‌ها',f.environments.join('، ')],['ایدهٔ نخست',f.ideaEnabled?f.ideaTitle:'بعداً ثبت می‌شود']].map(([a,b])=>'<article><small>'+escape(a)+'</small><strong>'+escape(b||'—')+'</strong></article>').join('')}
    ...
    async function poll(token){try{const res=await fetch('/api/status',{headers:{'x-product-ops-csrf':token}});const data=await res.json();renderStatus(data);if(data.status==='completed'){dashboardUrl=data.dashboardUrl;$('#running').hidden=true;$('#success').classList.add('show');const developmentReady=data.result.development&&data.result.development.status==='ready';const executorsReady=data.result.development&&data.result.development.executorsEnabled;$('#success-copy').textContent='پوشهٔ عملیات در '+data.result.operationsPath+' ساخته شد. '+(developmentReady?'سامانهٔ توسعه آماده و معتبر است. ':'')+(executorsReady?'نقش‌های مهندسی نیز به کدکس متصل شده‌اند. ':'')+'تا چند لحظهٔ دیگر وارد پنل می‌شوید.';setTimeout(()=>location.href=dashboardUrl,1800);return}if(data.status==='failed'){fail(data.error);return}setTimeout(()=>poll(token),700)}catch(error){fail(error.message)}}
