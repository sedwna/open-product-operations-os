...
function automationPanel(automation = {}) {
  ...
  return `<div class="automation-grid">
    ...
    <article class="panel automation-next"><span class="kicker">قدم بعدی سامانه</span><h2>برداشتن خودکار وظیفه</h2><p>${escapeHtml(automation.nextCapability || "زمان‌بند محلی و اجارهٔ وظیفه")}</p><div class="notice-line">تا پیش از فعال‌شدن این بخش، «متصل بودن کدکس» به معنی «شروع پیوستهٔ ساخت» نیست.</div></article>
  </div>`;
}
...
function clientScript() {
  ...
  (()=>{
    ...
    function render(){renderFlow();renderAttention();renderRoles();renderTasks();renderApprovals();renderIntake();renderRisks()}
    ...
    $('#run-cycle-button').onclick=async()=>{try{await api('/api/operate',{});await refresh();toast('چرخهٔ محصول اجرا و وضعیت تازه شد.')}catch(error){toast(error.message,true)}};
    ...
    render();if(window.__PRODUCT_OPS_LIVE__)setInterval(()=>refresh().catch(()=>{}),15000);
  })();`;
}
